...
impl MMap {
    ...
    pub(crate) unsafe fn m_map(fd: i32, len: usize, off: usize) -> InternalResult<Self> {
        let ptr = mmap(
            std::ptr::null_mut(),
            len,
    ...
    /// Unmap `[MMap]` region
    pub(crate) unsafe fn m_unmap(&self) -> InternalResult<()> {
    ...
    pub(crate) unsafe fn m_async(&self) -> InternalResult<()> {
    ...
    pub(crate) unsafe fn m_sync(&self) -> InternalResult<()> {
