...
fn test_concurrent_operations() -> TurboResult<()> {
        ...
        let handle = thread::spawn(move || -> TurboResult<()> {
            ...
            for j in 0..OPS_PER_THREAD {
                ...
                match op_type {
                    ...
                    // Delete operation
                    2 => {
                        if cache_clone.del(&key)?.is_some() {
    ...
    assert_eq!(cache.total_count()?, final_expected_keys.len());
...
fn test_crash_during_swap_with_staging() -> TurboResult<()> {
    ...
    // Phase 2: Restart cache and verify recovery
    {
        ...
        for (key, expected_value) in &dataset {
            let actual_value = cache.get(&key)?;
...
fn test_with_large_amounts() {
    let tmp = TempDir::new().unwrap();
    let dataset = gen_dataset(32);

    match std::fs::remove_dir_all(&tmp.path()) {
        Ok(_) => {}
        Err(_) => {}
    }

    let cache = TurboCache::new(tmp.path().to_path_buf(), INIT_CAP).unwrap();
    ...
    assert_eq!(cache.total_count().unwrap(), 32);

    // check if all items are retrived correctly
    for (k, v) in &dataset {
        let vbuf = cache.get(&k).unwrap();
